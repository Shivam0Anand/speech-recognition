  ...
  if (string) {
    ...
    request.execute(response => {
      ...
      data.items.forEach((item, index) => {
        ...
        var iframe = `<iframe src="//www.youtube.com/embed/${item.id.videoId}${
          index === 0 ? "?rel=0&autoplay=1" : ""
        }" height="300" width="600" allowfullscreen></iframe>`;
...
recognition.addEventListener("result", e => {
  ...
  if (e.results[0].isFinal) {
    ...
    if (
      speech.toLowerCase() === "clear youtube" ||
      speech.toLowerCase() === "remove youtube"
    ) {
